...
public:
    ...
    //Used with Exclusivesend. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const;
...
public:
    ...
    // Coin selection
    bool SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nExclusiveSendRoundsMin, int nExclusiveSendRoundsMax);
    ...
    /// Get 1000TRIVECHAIN output and keys which can be used for the Masternode
