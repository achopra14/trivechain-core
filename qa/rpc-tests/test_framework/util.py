...
# Copyright (c) 2014-2017 The Trivechain Core developers
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "trivechain.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def initialize_chain(test_dir):
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        for i in range(4):
            ...
            args = [ os.getenv("TRIVECHAIND", "trivechaind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print "initialize_chain: trivechaind started, waiting for RPC to come up"
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("TRIVECHAIND", "trivechaind")
    ...
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: trivechaind started, waiting for RPC to come up"
