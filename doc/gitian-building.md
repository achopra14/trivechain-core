...
*Setup instructions for a Gitian build of Trivechain Core using a Debian VM or physical system.*
...
- [Building Trivechain Core](#building-trivechain-core)
...
cd vm-builder-2.0.0.1.4+bzr494
...
Clone the git repositories for Trivechain Core and Gitian.
...
in the Trivechain Core repository under 'Fetch and build inputs' to install sources which require
...
To build Trivechain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Trivechain Core repository.
...
BTCPATH=/some/root/path/trivechain.git
SIGPATH=/some/root/path/trivechain-detached-sigs.git
...
[trivechain/gitian.sigs](https://github.com/trivechain/gitian.sigs/) repository, or if that's not possible to create a pull
request.
